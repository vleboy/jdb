...
namespace jdb
{
    public partial class Blocks : Form
    {
        ...
        private void Blocks_Load(object sender, EventArgs e)
        {
            ...
            laYardValue.Text = db.GetSingleObject("SELECT Count(block.id) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id WHERE district.id = "+ this.district +" AND grid.id = " + gridid).ToString();
            laFamilyValue.Text = db.GetSingleObject("SELECT Count(population.id) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id INNER JOIN features ON population.features = features.id INNER JOIN resident ON features.resident = resident.id INNER JOIN residentaddresss ON resident.resident_addresss = residentaddresss.id WHERE residentaddresss.`host` = 1 AND district.id = "+this.district+" AND grid.id = " + gridid ).ToString();
            laHouseValue.Text = db.GetSingleObject("SELECT Sum(street.building) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN street ON street.blockId = block.id WHERE district.id = " + this.district+" AND grid.id = " +gridid).ToString();
            laUnitValue.Text = db.GetSingleObject("SELECT Sum(street.floor) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN street ON street.blockId = block.id WHERE district.id = "+this.district+" AND grid.id = " + gridid).ToString();
            ...
            laCommunistValue.Text = db.GetSingleObject("SELECT Count(population.id) FROM district INNER JOIN grid ON grid.district = district.id INNER JOIN block ON block.grid = grid.id INNER JOIN population ON population.block = block.id WHERE population.educational = '党员' AND district.id = "+this.district+" AND grid.id =  " + gridid).ToString();
